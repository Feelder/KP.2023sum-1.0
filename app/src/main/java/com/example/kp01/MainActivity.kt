...
import com.example.kp01.databinding.ActivityMainBinding
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val networkUsage = NetworkUsageManager(this, com.example.kp01.Util.getSubscriberId(this))

        val handler = Handler(Looper.getMainLooper())
        val runnableCode = object : Runnable {
            override fun run() {
                val now = networkUsage.getUsageNow(com.example.kp01.NetworkType.ALL)
                val speeds = NetSpeed.calculateSpeed(now.timeTaken, now.downloads, now.uploads)
                val todayM = networkUsage.getUsage(Interval.today, com.example.kp01.NetworkType.MOBILE)
                val todayW = networkUsage.getUsage(Interval.today, com.example.kp01.NetworkType.WIFI)

                binding.wifiUsagesTv.text = "WiFi: " + com.example.kp01.Util.formatData(todayW.downloads, todayW.uploads)[2]
                binding.dataUsagesTv.text = "Mobile: " + com.example.kp01.Util.formatData(todayM.downloads, todayM.uploads)[2]
                binding.apply {
        ...
        val last30DaysWIFI = networkUsage.getMultiUsage(
            Interval.lastMonthDaily, com.example.kp01.NetworkType.WIFI
        )

        val last30DaysMobile = networkUsage.getMultiUsage(
            Interval.lastMonthDaily, com.example.kp01.NetworkType.MOBILE
        )

        for (i in last30DaysWIFI.indices) {
            usagesDataList.add(
                UsagesData(
                    com.example.kp01.Util.formatData(
        ...
        val last7DaysTotalWIFI = networkUsage.getUsage(
            Interval.last7days, com.example.kp01.NetworkType.WIFI
        )

        val last7DaysTotalMobile = networkUsage.getUsage(
            Interval.last7days, com.example.kp01.NetworkType.MOBILE
        )

        val last30DaysTotalWIFI = networkUsage.getUsage(
            Interval.last30days, com.example.kp01.NetworkType.WIFI
        )

        val last30DaysTotalMobile = networkUsage.getUsage(
            Interval.last30days, com.example.kp01.NetworkType.MOBILE
        )
        ...
        binding.wifiDataThisMonth.text = com.example.kp01.Util.formatData(
            last30DaysTotalWIFI.downloads,
            last30DaysTotalWIFI.uploads
        )[2]

        binding.mobileDataThisMonth.text = com.example.kp01.Util.formatData(
